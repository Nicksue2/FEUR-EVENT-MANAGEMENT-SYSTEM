...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("burger-btn")?.addEventListener("click", () => {
    ...
    if (sidebar)
      sidebar.classList.toggle(window.innerWidth <= 768 ? "open" : "minimized");
  });
  ...
  if (sessionData && sessionData.session) {
    ...
    if (notifList) {
      notifList.innerHTML = `
                <p class="notif-item" style="padding: 10px; border-bottom: 1px solid var(--border-color); font-size: 13px;">🔔 Welcome to FEUR Events!</p>
                <p class="notif-item" style="padding: 10px; border-bottom: 1px solid var(--border-color); font-size: 13px;">✅ Email Outlook sync is enabled.</p>
            `;
    }
    ...
    for (const event of eventsToRender) {
      ...
      card.innerHTML = `
                ...
                <div class="event-info">
                    ...
                    <div class="event-meta" style="font-size: 13px; color: gray; margin-bottom: 15px; display: flex; flex-direction: column; gap: 5px;">
  ...
  if (path.includes("admin.html") && userRole === "admin") {
    const fetchAdminEvents = async () => {
          ...
          .map(
            (ev) => `
                    <tr>
                        ...
                        <td>${ev.event_date}</td>
                        <td>
                            <button class="btn btn-solid" style="background:#facc15; padding:5px 10px;" onclick="window.editEvent('${ev.id}')">Edit</button>
      ...
      if (document.getElementById("stat-orders"))
        document.getElementById("stat-orders").innerText = regCount || 0;
    };
  ...
  if (ordersGrid && path.includes("orderlist.html")) {
    const { data: orders, error } = await supabase
      .from("orders")
      .select(
        `id, status, events ( title, event_date, event_time, campus, poster_url )`,
      )
      .eq("user_id", currentUser.id);
    ...
    if (error || !orders || orders.length === 0) {
      ...
      orders.forEach((order) => {
        ...
        card.innerHTML = `
                    ...
                    <div class="event-info">
                        <span class="status-badge" style="display: inline-block; padding: 5px 10px; border-radius: 12px; font-size: 12px; font-weight: bold; background: #dcfce7; color: #166534; text-align: center; margin-bottom: 10px; width: max-content;">${order.status}</span>
                        <div class="event-title" style="font-size: 18px; font-weight: bold; margin-bottom: 10px; color: var(--text-color);">${event.title}</div>
                        <div class="event-meta" style="font-size: 13px; color: gray; margin-bottom: 15px; display: flex; flex-direction: column; gap: 5px;">
                            <span>📅 ${event.event_date || "TBA"} | ${event.event_time || ""}</span>
                            <span>📍 FEU Roosevelt ${event.campus}</span>
                        </div>
